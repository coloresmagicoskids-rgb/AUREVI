...
import { WORLD_LABELS } from "../worlds/worldLabels";
...
function HomeFeed() {
  ...
  const recommendedCategories = Array.from(
    new Set([
      ...moodToCategories(viewerMood),
      ...reactionToCategories(topReaction),
    ])
  );
  ...
  useEffect(() => {
    async function fetchVideos() {
      ...
      const { data, error } = await supabase
        .from("videos")
        .select("*")
        .eq("world_type", activeWorld) // 👈 solo videos del mundo activo
  ...
  const handleDelete = async (videoId) => {
    ...
    try {
      ...
      if (dbError) {
        ...
        setErrorMsg("No se pudo eliminar el video de la base de datos.");
        return;
      }
  ...
  let filteredVideos =
    feedMode === "following"
      ? videos.filter(
          (v) =>
            currentUser && v.user_id && followingIds.has(v.user_id)
        )
      : videos;
  ...
  return (
    <section className="aurevi-screen aurevi-home-screen">
      ...
      <div className="aurevi-home-filters">
        <div className="profile-mode-toggle">
          ...
          <button
            type="button"
            className={
              "profile-mode-btn" + (feedMode === "following" ? " active" : "")
            }
      ...
      {loading && (
        <p className="aurevi-home-status">Consultando videos...</p>
      )}
        ...
        {filteredVideos.map((video) => {
          ...
          const isFollowing =
            currentUser && video.user_id
              ? followingIds.has(video.user_id)
              : false;
          ...
          const myReaction =
            currentUser
              ? listReactions.find((r) => r.user_id === currentUser.id)
                  ?.reaction || null
              : null;
          ...
          return (
            <article key={video.id} className="aurevi-feed-card">
              ...
              {video.description && (
                <p className="aurevi-feed-description">
                  {video.description}
                </p>
              )}
              ...
              >
                <button
                  ...
                  style={{
                    ...
                    fontSize: 14,
                    background:
                      "linear-gradient(90deg, #ff7aa2, #ffb347, #ffd452)",
                  }}
                >
                  ❤️ Me gusta
                </button>
                ...
                <span style={{ fontSize: 14, opacity: 0.9 }}>
                  {video.likes ?? 0} me gusta
                </span>

                <span style={{ fontSize: 14, opacity: 0.9 }}>
                  👁️ {video.views ?? 0} vistas
                </span>
